export interface DateChunk {
  dataInicialStr: string; // Formato YYYYMMDD
  dataFinalStr: string;   // Formato YYYYMMDD
}
 ...
 */
export function gerarBlocosDeData(diasRetroativosTotal: number = 30, maxDiasPorBloco: number = 14): DateChunk[] {
  const blocos: DateChunk[] = [];
  const hoje = new Date();
