...
export interface ExploradorFiltros {
  ...
  status?: string; // 'recebendo_proposta' | 'em_julgamento' | 'encerradas' | 'vigente' | 'todos'
  ufs?: string[];
  municipios?: string[];
  modalidades?: string[];
  orgaos?: string[];
...
export class PncpExplorerService {
  /**
   * Obtém um edital específico do PNCP pelo seu número de controle
   */
  static async obterPorId(tenantId: string, id: string) {
   ...
   */
  static async explorarPNCP(tenantId: string, filtros: ExploradorFiltros) {
    ...
    try {
      ...
      if (filtros.ufs && filtros.ufs.length > 0) {
        ...
        rawItems = rawItems.filter((item: any) => item.uf && ufsUpper.includes(String(item.uf).toUpperCase()));
      }
      ...
      if (filtros.municipios && filtros.municipios.length > 0) {
        ...
        if (!hasIds) {
          ...
          rawItems = rawItems.filter(
            (item: any) => item.municipio_nome && municsLower.includes(String(item.municipio_nome).toLowerCase())
          );
        }
      }
      ...
      const numeroControles = rawItems.map((item: any) => item.numero_controle_pncp).filter(Boolean);
   ...
   */
  static async salvarNoFunil(
    tenantId: string,
    itemPncp: any,
    novoStatus: 'EM_ANALISE' | 'SALVA' | 'DESCARTADA' = 'SALVA'
  ) {
